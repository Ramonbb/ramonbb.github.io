...
function createPublicationActions(publication, hasAbstract) {
  const actions = createElement("div", "card-actions");

  if (hasAbstract) {
    const toggle = createElement("button", "button small secondary abstract-toggle");
    toggle.type = "button";
    toggle.setAttribute("aria-expanded", "false");
    const icon = createElement("span", "", "▤");
    icon.setAttribute("aria-hidden", "true");
    const label = createElement("span", "", getTranslation(currentLanguage, "publications.abstract"));
    label.dataset.i18n = "publications.abstract";
  ...
  const actionConfig = [
    ...
    ["interactiveAppendix", "publications.interactiveAppendix", "▦", "ghost"],
    ["github", "publications.github", "⌘", "ghost"],
    ["dataset", "publications.dataset", "▥", "ghost"],
    ["citation", "publications.citation", "§", "ghost"]
  ];
...
function createPublicationCard(publication) {
  ...
  if (abstractText) {
    ...
    abstract.append(
      createElement("h3", "", getTranslation(currentLanguage, "publications.abstract")),
      createElement("p", "", abstractText)
    );
    card.append(abstract);
  }

  const actions = createPublicationActions(publication, Boolean(abstractText));
